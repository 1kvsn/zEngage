					...
					if (user && req.body.isInvited) {
						Invite.findOneAndUpdate({ teammateEmail: req.body.email }, { isVerified: req.body.isInvited }, (err, updatedTeammate) => {
							if (err) {
								return res.status(500).json({
									success: false,
									message: 'Unable to update the invited member!'
								});
							}
							if (updatedTeammate) {
								console.log(updatedTeammate.org, 'updatedTeammate.org');
								Org.findOneAndUpdate({ _id: updatedTeammate.org[0] }, {
									$push: {
										members: user._id,
									}
								}, (err, updatedOrg) => {
									...
									if (err) {
										res.status(500).json({
											success: false,
											message: 'Encountered error while updating members in Org',
											err
										})
									} else {
										return res.status(200).json({
											success: true,
											message: 'Invited member registered successfully'
										});
									}
								})
							}
